...
public class JwtTokenUtil implements Serializable {
    ...
    private String doGenerateToken(final User user) {
        ...
        claims.put(AppConstants.SCOPES, user.getRoles().stream().map(Roles::getRole).collect(Collectors.toList()));

        byte[] apiKeySecretBytes = DatatypeConverter.parseBase64Binary(AppConstants.SECRET_KEY);
        Key signingKey = new SecretKeySpec(apiKeySecretBytes, signatureAlgorithm.getJcaName());

        return Jwts.builder().setClaims(claims).setIssuer(AppConstants.EMBL).setIssuedAt(new Date(System.currentTimeMillis()))
